...
describe('ec-encrypt', function () {
  ...
  context('#generateToken', function () {
    ...
    it('should accept numerical parameters but convert them to string', function () {
      var key = 1234;
      var expireTime = 5678;

      _ecEncrypt2.default.generateToken(key, expireTime);

      _chai.assert.equal(_typeof(_ecEncrypt2.default.generateHash.args[0][0]), _typeof('string'), 'type of argument passed to generateHash should match string type');
      _chai.assert.deepEqual(_ecEncrypt2.default.ecEncrypt.args[0][2], new Buffer(String(expireTime)), 'expireTime should have been converted to a string before adding to buffer');
    });
  });
